...
export default function Carreras() {
  ...
  const [administrador , setAdministrador ]=useState({
   ...
   const columns = [
    { id: 'nombre', label: 'Carrera', minWidth: "25%" },
    { id: 'action', label: 'Acción',  minWidth: "25%",  },
  ];

  function createData( nombre, action) {
  ...
  const peticionGet=async()=>{
    await axios.get('http://localhost/GestionPracticas_G4/ci-practicas-back/public/getCarrerasTabla')
    .then(response=>{
      console.log(response.data)
      const resultado = response.data;
      const lista = []
      for(var i=0; i<resultado.length; i++){
        const fila = createData(resultado[i].nombre ,  "button")
        lista.push(fila)
      }  
      setRows(lista)
    })
...
.nombre}</b> ? </p>
  ...
  return (
    ...
    <div className={classes.encabezado}>
      ...
      >
        Carreras</Typography></motion.div>
    </div>
     
    <Button className={classes.boton} onClick={()=>abrirCerrarModalInsertar()}>Agregar Carrera</Button>
      <br /><br />
      <hr/> 
    ...
    <Dialog open={modalInsertar} onClose={abrirCerrarModalInsertar} aria-labelledby="form-dialog-title" >
          <DialogTitle id="form-dialog-title">Nuevo Carrera</DialogTitle>
          <DialogContent>         
          {bodyInsertar}
          </DialogContent>
        </Dialog>
