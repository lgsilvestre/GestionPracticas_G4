...
class PracticaModel extends Model
{
    ...
    public function getInfoAlumnoPractica($id,$np){
      $queryPracticaAlumno = "SELECT * FROM practica INNER JOIN alumno ON practica.refAlumno = alumno.id_alumno WHERE alumno.id_alumno = ".$id." ORDER BY CASE WHEN etapa = 'Solicitud' THEN 1 ELSE 2 END, etapa";
      $query = $this->db->query($queryPracticaAlumno);
      $result = $query->getResult();
      return $result;
    }
    public function getInfoPracticaById($id){
      $queryPracticaAlumno = "SELECT * FROM practica INNER JOIN alumno ON practica.refAlumno = alumno.id_alumno WHERE practica.id_practica = ".$id." ";
      $query = $this->db->query($queryPracticaAlumno);
      $result = $query->getResult();
      return $result;
    }
    ...
    public function getEstadoAlumno($id,$numero){
        $queryPracticaAlumno = "SELECT etapa, estado FROM practica where practica.refAlumno = ".$id." and practica.numero = ".$numero." ORDER BY id_practica DESC LIMIT 1";
        $query = $this->db->query($queryPracticaAlumno);
        $result = $query->getResult();
        return $result;
    }
    public function denegarPractica($id){
        $queryPracticaAlumno = "UPDATE practica SET activa = 0, etapa = 'Rechazada', estado='Rechazada' where practica.refAlumno = ".$id." and practica.activa = 1";
    ...
    public function recahzarSolicitud($id){
        //$queryPracticaAlumno = "SELECT etapa, estado FROM practica where practica.refAlumno = ".$id." and practica.numero = ".$numero;
        $queryPracticaAlumno = "UPDATE practica SET estado = 'Rechazada', activa='0' where practica.refAlumno = ".$id." and practica.activa = '1'";
        $this->db->query($queryPracticaAlumno)->getResult();
        $result = $this->db->affectedRows();
        return $result;
    }    
    public function RechazarEtapa($id,$etapa){
        //$queryPracticaAlumno = "SELECT etapa, estado FROM practica where practica.refAlumno = ".$id." and practica.numero = ".$numero;
        $queryPracticaAlumno = "UPDATE practica SET estado = 'Rechazada' where practica.refAlumno = ".$id." and practica.activa = '1'";
        $this->db->query($queryPracticaAlumno)->getResult();
        $result = $this->db->affectedRows();     
        return $result;
    }    
    ...
    public function inscribir($nro_practica, $id_alumno, $empresa, $supervisor, $fch_inicio, $fch_termino,$rut_empresa,$email_supervisor, $tel_supervisor,$region_empresa,$zona_empresa,$tel_emer,$nombre_emer){
        $queryPracticaAlumno = "UPDATE practica SET
        estado = 'Por inscribir',  
        fecha_inicio = '".$fch_inicio."', 
        fecha_termino = '".$fch_termino."', 
        empresa = '".$empresa."', 
        supervisor = '".$supervisor."', 
        rut_empresa = '".$rut_empresa."', 
        email_supervisor = '".$email_supervisor."', 
        telefono_supervisor = '".$tel_supervisor."', 
        region = '".$region_empresa."', 
        comuna = '".$zona_empresa."', 
        nombre_contacto = '".$nombre_emer."', 
        telefono_contacto = '".$tel_emer."' 
        where 
        practica.refAlumno = '".$id_alumno."' 
        and practica.numero='".$nro_practica."'";
